...
const KEYS={progress:'nivalis.campaign.progress.v1',settings:'nivalis.campaign.settings.v1',mementos:'nivalis.campaign.mementos.v1'};
let storage=true;
...
async function chapterDocument(key){
 ...
 if(!requests.has(key))requests.set(key,(async()=>{
  ...
  const html=embedded?new TextDecoder().decode(Uint8Array.from(atob(embedded),c=>c.charCodeAt(0))):await fetch(`${base}${key}.html?v=20260920-privacy1`).then(r=>{if(!r.ok)throw new Error(`Chapter download failed (${r.status}).`);return r.text();});
